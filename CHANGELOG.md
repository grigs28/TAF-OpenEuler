...
- 月度任务调度改为周期语义（day_of_month 日→下月前一天为一个周期），跨月补跑不再跳月；调度器主循环、动作处理器的月度去重统一改为周期判断
- 修复任务成功/失败统计 UPDATE 缺 commit 导致重启后丢失成功记录、月任务重复触发的问题
- 修复成功路径 UPDATE 复用占位符 $2 导致 "8 placeholders but 7 parameters" 报错（转换器按出现次数转换）
- 修复 skipped 结果被当作成功计入统计并污染 last_success_time 的问题
- 上次执行时间（last_run_time）只在任务成功时写入，启动/失败/跳过不再覆盖
- 数据库连接池：check 借出前快速存活检查（3s 超时判死）+ tcp_user_timeout=5s，修复长时间空闲后首次打开页面数据库连不上（中间设备静默断链）的问题
- SSO 会话持久化到 temp/yz_sessions.json，服务重启不再强制全员重新登录
- 新增全局 authFetch：401 自动跳转 SSO 重新登录，首页 API 全部接入
- 计划任务操作历史：scheduler_run 区分"自动运行/手动运行"（依据日志中的 manual_run 标记），旧记录显示"执行任务"
...
- 系统日志页面：合并日志级别按钮和下拉条，按钮同时控制运行时级别和显示过滤，操作日志也受 level 过滤
- 修复退出后跳转到 yz-login 登录页的问题：改为回到 TAF 首页
- 系统设置页面新增"单点登录 (SSO)"配置卡片（yz-login 地址、回调地址）
- 启动时自动检测本机局域网 IP（不再硬编码 192.168.0.28）
- 备份扫描错误文件入库：`BackupFile` 表加 `error_message` 列，扫描失败的文件记录到数据库
...
- 修复恢复页面所有 fetch 请求未携带 SSO Cookie 导致 401 的问题（`credentials: 'include'`）
- 修复 LTFS 挂载第三重问题：移除 `allow_root` 选项（导致 FUSE 守护进程崩溃）
- 修复"退出"按钮无效：登出改为两阶段（先清 yz-login SSO 会话，再清 TAF Cookie），防止自动重新登录
- 修复 SSO 登出后首页 `/` 不触发重新认证的问题：将 `/` 加入 SSO 白名单
- 修复首页所有 fetch 请求未携带 SSO Cookie 的问题
- 修复管理面板退出登录同样无效的问题
...
- 修复恢复页面 LTFS 挂载失败：`_get_ltfs_device()` 自动检测 SCSI generic 设备时，通过 sysfs 精确定位磁带机的 sg 设备（而非返回首个存在的 sg 设备）
- 修复备份页面 `from_scheduler` 标记对执行记录始终为 false 的问题：JOIN 条件加入 `bt.template_id` 和 JSON 兜底匹配
- 修复"所有任务"表格中验证任务完成后仍显示"准备中..."的问题
